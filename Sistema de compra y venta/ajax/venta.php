...
switch ($_GET["op"]){
	...
	case 'listarArticulosVenta':
		...
		$rspta=$articulo->listarActivosVenta();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				"0"=>(($reg->stock==0)?'<button class="btn btn-danger" onclick="nostock()"> <span class="fa fa-plus"></span></button>':'<button class="btn btn-success" onclick="agregarDetalle('.$reg->idarticulo.',\''.$reg->nombre.'\',\''.$reg->precio_venta.'\',\''.$reg->stock.'\')"><span class="fa fa-plus"></span></button>'),
 				"1"=>$reg->nombre,
 				"2"=>$reg->categoria,
 				"3"=>$reg->codigo,
 				"4"=>$reg->stock,
